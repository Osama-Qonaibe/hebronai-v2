...
async function migrateImageLimits() {
  ...
  try {
    ...
    for (const plan of plans) {
      ...
      if (limits?.images) {
        console.log(`  ✅ ${plan.name} (${plan.slug}) - Already has image limits`);
        skipped++;
        continue;
      }
      
      try {
        console.log(`  ⚙️  ${plan.name} (${plan.slug}) - Adding image limits...`);
        ...
        if (features?.advanced?.imageGeneration) {
          ...
          } else if (plan.slug.includes('enterprise') || plan.slug.includes('unlimited')) {
            imageLimits = {
              maxPerDay: 200,
              maxPerMonth: 2000,
              maxResolution: "1792x1024"
    ...
    console.log('━'.repeat(60));
    console.log(updated > 0 ? '✅ Migration completed successfully!' : 'ℹ️  No plans needed updating.');
